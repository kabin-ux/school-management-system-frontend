...
import { createSalarySchema, type SalaryStructure } from "../../../../zod-schema/salary";
...
interface SalaryStructureModalProps {
    ...
    onClose: () => void;
    onSubmit: (feeData: SalaryStructure) => void;
    isLoading: boolean;
}
...
export const AddSalaryStructureModal: React.FC<SalaryStructureModalProps> = ({
    ...
    const { register, handleSubmit, reset, formState: { errors, isSubmitting }, clearErrors, setError } = useForm<SalaryStructure>({
    ...
    const onFormSubmit = async (data: SalaryStructure) => {
        try {
            ...
            setError("root", {
                message: "Failed to add salary structure. Please try again.",
            });
        }
    };
